...
- [x] 4.2 A non-positive silence threshold disables suspension (always-on
      streaming) so the feature can be switched off without a code change.

## 5. Tests

- [x] 5.1 Governor: silence past the threshold yields `suspend`; speech
      before the threshold cancels the countdown.
- [x] 5.2 Governor: speech while suspended yields `resume` and the pre-roll
      buffer is returned first, oldest sample first.
- [x] 5.3 Governor: no `suspend` while speech is ongoing, and the pre-roll
      buffer stays bounded.
- [x] 5.4 Transcriber/metrics: streamed vs. suppressed seconds, saving
      fraction, latency EMA, final-token ratio, end-cause attribution,
      redacted error, speaker count.
- [x] 5.5 Suspend sends the terminator exactly once.
- [x] 5.6 `swiftc -typecheck` against the real sources is clean, and
      SwiftLint/SwiftFormat pass.

## 6. Documentation

- [x] 6.1 Write `docs/SONIOX-BETA-PLAN.md`: current state with file
