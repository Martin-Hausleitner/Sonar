...
final class LatencyBudgetTests: XCTestCase {
    func testFrameSizeIs10msAt48kHz() {
        ...
        XCTAssertEqual(LatencyBudget.audioSampleRate, 48000)
...
final class MetricsTests: XCTestCase {
    func testMachTimebaseConversion() {
        ...
        XCTAssertEqual(ms, 1000.0, accuracy: 0.5)
    }
    ...
    func testTraceMarkAndElapsedRoundTrip() throws {
        let m = Metrics()
        let id = m.openTrace()
        m.mark(id, .captured)
        ...
        let trace = try XCTUnwrap(m.snapshot().first { $0.frameID == id })
...
final class E2EAudioPipelineTests: XCTestCase {
    func testPCMBufferAt48kHzMonoMatchesFrameSize() throws {
        ...
        let buf = try XCTUnwrap(AVAudioPCMBuffer(
            pcmFormat: format,
        ...
        XCTAssertEqual(buf.format.sampleRate, 48000)
    ...
    func testSineFillSurvivesRoundTripSetup() throws {
        ...
        let ptr = try XCTUnwrap(buf.floatChannelData?[0])
        let twoPi = 2 * Float.pi
        let freqHz: Float = 1000
