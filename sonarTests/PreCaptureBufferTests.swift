...
final class PreCaptureBufferTests: XCTestCase {
    ...
    func testDrainReturnsFramesOldestFirst() throws {
        ...
        XCTAssertEqual(try XCTUnwrap(drained[0].floatChannelData?[0][0]), 0.0, accuracy: 1e-6)
        XCTAssertEqual(try XCTUnwrap(drained[1].floatChannelData?[0][0]), 0.1, accuracy: 1e-6)
        XCTAssertEqual(try XCTUnwrap(drained[2].floatChannelData?[0][0]), 0.2, accuracy: 1e-6)
    }

    // MARK: - Over-capacity eviction

    func testPushOverCapacityEvictsOldest() throws {
        let pcb = PreCaptureBuffer(durationMs: 30, frameMs: 10) // capacity = 3
        ...
        XCTAssertEqual(
            try XCTUnwrap(drained[0].floatChannelData?[0][0]),
            0.1,
            accuracy: 1e-6,
    ...
    func testPushFarOverCapacityRetainsOnlyMostRecent() throws {
        ...
        XCTAssertEqual(try XCTUnwrap(drained[0].floatChannelData?[0][0]), 0.8, accuracy: 1e-6)
        XCTAssertEqual(try XCTUnwrap(drained[1].floatChannelData?[0][0]), 0.9, accuracy: 1e-6)
    }
